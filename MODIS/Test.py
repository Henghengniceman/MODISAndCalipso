# -*- coding: utf-8 -*-
"""
Created on Fri Dec  3 09:10:41 2021
...
for filename in tqdm(os.listdir('../Data/Terra/'+Day)):
    ...
    ac = ax.contourf(Longitude,Latitude, AOD,vim=0,vmax=0.4)
    
    ax.set_xlim(-30,70)
    ax.set_ylim(0,90)
    AOD  = list(AOD)
    ax.coastlines()
    ax.set_title('MODIS AOD @ 550 nm, 2021-'+Day[4:6]+'-'+Day[6:8],fontdict = fontdicts)
    ...
    if not os.path.exists('../Figure/Terra/'+Day):
            os.makedirs('../Figure/Terra/'+Day)
    plt.savefig('../Figure/Terra/'+Day+'/AOD'+filename+'.png')
    plt.close()